    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(unet, vae_ctp):
                ...
                # Mask 重建为 CTP 图像后的损失，主要用于训练 CTP VAE 的 Decoder 输出层。
                z0_predicted = perturb_latent - t * model_pred
                z0_predicted_unscaled = z0_predicted * (1.0 / vae_ctp.config.scaling_factor)
                recon_ctp = vae_ctp.decode(z0_predicted_unscaled.to(weight_dtype)).sample
                recon_loss = F.mse_loss(recon_ctp.float(), ctp_images.float(), reduction="mean")
                
                loss = rf_loss + 1.0 * self_recon_loss + 1.0 * recon_loss
                ...
                avg_recon_loss = accelerator.gather(recon_loss.repeat(args.train.batch_size)).mean()
